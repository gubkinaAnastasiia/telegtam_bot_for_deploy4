...
@Component
public class ProcessingButton implements ResponceProcessing{
    ...
    public void dataAcceptance(Update update) {
        ...
        switch (botState) {
            case TEST_SELECTION:
                Test test = getTest(buttonQuery.getData());
                if (test == null) {
                ...
                break;
            case CONSENT_TEST:
                if (buttonQuery.getData().equals(ButtonState.YES.name())) {
                    userDataCache.setUserCurrentBotState(userId, BotState.TEST);
                } else if (buttonQuery.getData().equals(ButtonState.NO.name())) {
                    userDataCache.setUserCurrentBotState(userId, BotState.SHOW_MAIN_MENU);
                } else {
                ...
                break;
            case TEST:
                if (userDataCache.getUserTest(userId).checkAnswer(buttonQuery.getData())) {
                    getAnswer(userId, buttonQuery.getData());
    ...
    public Test getTest(String answer) {
        Test test = null;
        switch (answer) {
            case "TEST_BECK": test = new TestBeck();
        }
        return test;
    }
